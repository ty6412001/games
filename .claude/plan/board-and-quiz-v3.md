# 棋盘重排 + 地产答题 + 小朋友科目轮换 + 答对题去重 实施计划 (v3)

> **Task**: 四项改动一起落地。(A) **每局开局随机洗牌**生成 28 格棋盘；(B) 踩到别人地"答题免租"可选，踩到自己地"答题领奖励"可选（业主不分钱、自地奖金 `min(80, baseRent×0.5)`）；(C) 小朋友答题**一步冷却**：上一题选过的语/数/英学科下次禁用；(D) 验证 + 巩固"答对的题目后续不再出现"规则，并让新 Quiz Context（property-rent / property-bonus）也参与该去重。
...
### D. 答对题去重（已基本就位，补测 + 接入新 context）

**现状确认**（`gameStore.ts:361-370`）：
```ts
const pickQuestionForSubject = (
  ...
  const fresh = pickRandomQuestion(pack, { subject, excludeIds: subjectCorrect });
  if (!fresh) return null;
  return { question: fresh };
};
```
`selectSubject`（所有 QuizContext 的入口）→ `pickQuestionForSubject` → `pickRandomQuestion(..., excludeIds)`。因此 `property-rent` / `property-bonus` 也**自动跳过已答对的题**。

**需要动的地方**：
1. `SubjectSelector.availCount` 当前按**题包总数**算，不是**可用数**。为配合新 rule 与 UX 诚实显示，改成排除 `correctQuestionIds[subject]` 后的剩余量。
2. 补一条集成测：答对一题 → 刷新同学科 picker → 不会再抽到那个 id。
...
### D 组（答对题去重验证 + UI 诚实计数，2 步）

15. **Modify `SubjectSelector.availCount`（步骤 12 里顺道做）**：
    ```ts
    const availCount = (subj: Subject): number => {
      if (subj === 'brain') return brainQuestionCount;
      const correctSet = correctQuestionIds[subj];
      return pack.questions.filter(
        (q) => q.subject === subj && !correctSet.has(q.id),
      ).length;
    };
    ```

16. **Add test `apps/web/src/stores/__tests__/correctExclusion.test.ts`**：
    - 答对题 q1 后，`correctQuestionIds.math` 含 q1
    - 再次 `selectSubject('math')` → `activeQuiz.question.id !== 'q1'`（通过题包只有一题的场景触发 `null` 返回）
    - 所有 QuizContext 都走同一 picker，`property-rent` 场景同样成立
...
| `apps/web/src/stores/gameStore.ts` | Modify | + `property-rent` / `property-bonus` QuizContext、4 个新 action、`lastChildSubject`、`submitAnswer` 分支、`cancelPendingQuiz` 白名单 |
...
| `apps/web/src/stores/__tests__/correctExclusion.test.ts` | Create | 答对题去重端到端测 |
...
**Plan generated and saved to `.claude/plan/board-and-quiz-v3.md`**
...
```
/ecc:multi-execute .claude/plan/board-and-quiz-v3.md
```
